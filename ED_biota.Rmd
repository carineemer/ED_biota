...
taxonomy<-join(ita_rpool, valesca, by = "species", type = "left")
...
write.csv(taxonomy2, "ita_plants_taxonomy.csv")
...
result_plants<-S.PhyloMaker(splist=ita_rpool_tax, tree=phylo, nodes=nodes)
...
PolytomyResolver(tree_plants,file="tree_plants_ita") ### tree_plants.xml goes to BEAST - takes a few hours

### then, generates tree_plants.trees which should be read again here to run the following:

trees<-read.nexus("tree_plants_ita.trees") ## huge file
...
tt100_1<-tt100[[1]]
plot(tt100_1, cex = 0.5)
write.tree(tt100,"ita_plants_100trees.nwk")
...
write.csv(ED100_p,"ita_plants_ED100.csv")
...
mean_ed_ita<-ED100_p[,-(2:101)]
colnames(mean_ed_ita)[1]<-"species"

# check names
setdiff(tt100_1$tip.label, colnames(ITA_com))
setdiff(colnames(ITA_com), tt100_1$tip.label)

colnames(ITA_com)[which(names(ITA_com) == "Dalbergia_frutescens_var._frutescens")] <- "Dalbergia_frutescens"

### combine mean_ed_car$mean_ed in the CAR_com
mtx1<-as.matrix(ITA_com)
list1<-matrix2sample(mtx1)
colnames(list1)[3]<-"species"
treat_ed<-join(list1,mean_ed_ita, by= "species", type = "left")

write.csv(treat_ed, "ITA_ED.csv")
